...
export default function DatabasePage({ tab = 'tables' }: { tab?: Tab }) {
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      fd.append('file', file);
      const r = await fetch(`${apiUrl}/api/import-export/import-employees`, {
        method: 'POST',
        headers: { ...authHeader() }, // pas de Content-Type : le navigateur gère le multipart
        body: fd,
      });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) { showToast('error', d.detail || 'Échec de l\'import.'); return; }
      setImportResult(d);
      showToast('success', `Import terminé : ${d.success}/${d.total} ligne(s).`);
      ...
      {tab === 'import' && (
        <section className="db-section">
          <div className="db-import-card">
            ...
            <h2>Importer des employés (Excel)</h2>
            <p>
              Téléchargez d'abord le modèle, remplissez-le, puis importez le fichier <code>.xlsx</code>.
              Les colonnes doivent correspondre exactement au modèle.
            </p>

            <div className="db-import-actions">
              <button
                className="db-btn ghost"
                onClick={() => downloadFile(`${apiUrl}/api/import-export/download-template`, 'modele_import_employes.xlsx')}
              >
          ...
          {importResult && (
            <div className="db-import-result">
              <div className="db-result-stats">
                <span className="db-stat ok"><CheckCircle size={16} /> {importResult.success} importé(s)</span>
                <span className="db-stat total">{importResult.total} ligne(s) lues</span>
                {importResult.errors.length > 0 && (
                  <span className="db-stat err"><AlertCircle size={16} /> {importResult.errors.length} erreur(s)</span>
                )}
              </div>
              {importResult.errors.length > 0 && (
                <ul className="db-error-list">
                  {importResult.errors.slice(0, 50).map((e, i) => (
                    <li key={i}>{e.row ? `Ligne ${e.row} : ` : ''}{e.error}</li>
                  ))}
                </ul>
              )}
            </div>
